...
function LogRollFormScreen() {
  ...
  const existing = useAppSelector(state =>
    rollId ? state.rolls.items.find(r => r.id === rollId) : undefined,
  );
  ...
  const handleSave = async () => {
    ...
    const result = existing
      ? await dispatch(updateRoll({ id: existing.id, changes }))
      : await dispatch(createRoll(changes));
    setSaving(false);
    if (
      result.type === createRoll.fulfilled.type ||
      result.type === updateRoll.fulfilled.type
    ) {
      navigation.goBack();
    } else {
      Alert.alert('Something went wrong', 'Could not save this roll.');
    }
  };
  ...
  const handleDelete = () => {
    ...
    Alert.alert('Delete roll?', 'This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          await dispatch(deleteRoll(existing.id));
          navigation.goBack();
        },
      },
    ]);
