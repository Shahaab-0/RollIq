...
import * as competitionsApi from '../api';
...
export function useCreateCompetition() {
  ...
  return useMutation({
    ...
    onError: error => showToast(getApiErrorMessage(error, 'Could not save this competition'), 'error'),
...
export function useDeleteCompetition() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['competitions'] }),
    onError: error => showToast(getApiErrorMessage(error, 'Could not delete this competition'), 'error'),
...
export function useCreateMatch(competitionId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (match: NewCompetitionMatch) => competitionsApi.createMatch(competitionId, match),
    onSuccess: () => {
...
export function useUpdateMatch(competitionId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ matchId, changes }: { matchId: string; changes: Partial<NewCompetitionMatch> }) =>
...
export function useDeleteMatch(competitionId: string) {
  ...
  return useMutation({
    ...
    onError: error => showToast(getApiErrorMessage(error, 'Could not delete this match'), 'error'),
