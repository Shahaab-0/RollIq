...
import * as gymsApi from '../api';
import { showToast } from '@/lib/toast';
import { getApiErrorMessage } from '@/lib/apiClient';
import type { GymRole, NewGym, NewGymClassEntry, NewGymClassVideo, NewGymScheduleEntry } from '../types';

export function useGyms() {
  return useQuery({ queryKey: ['gyms'], queryFn: gymsApi.listGyms });
}
...
export function useJoinGym() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['gyms'] }),
    onError: error => showToast(getApiErrorMessage(error, 'Could not join with that code'), 'error'),
...
export function usePromoteMember(gymId: string) {
  ...
  return useMutation({
    ...
    onError: error => showToast(getApiErrorMessage(error, "Could not update this member's role"), 'error'),
...
export function useCreateScheduleEntry(gymId: string) {
  ...
  return useMutation({
    ...
    onError: error => showToast(getApiErrorMessage(error, 'Could not add this schedule slot'), 'error'),
...
export function useDeleteScheduleEntry(gymId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['gyms', gymId, 'schedule'] }),
    onError: error => showToast(getApiErrorMessage(error, 'Could not remove this schedule slot'), 'error'),
...
export function useMarkAttendance(gymId: string, classId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['gyms', gymId, 'classes', classId, 'attendance'] }),
    onError: error => showToast(getApiErrorMessage(error, 'Could not mark attendance'), 'error'),
  });
}
...
export function useUnmarkAttendance(gymId: string, classId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['gyms', gymId, 'classes', classId, 'attendance'] }),
    onError: error => showToast(getApiErrorMessage(error, 'Could not update attendance'), 'error'),
  });
}
