...
export function useReplaceSessionTechniques() {
  ...
  return useMutation({
    mutationFn: ({ sessionId, techniqueIds }: { sessionId: string; techniqueIds: string[] }) =>
      sessionsApi.replaceSessionTechniques(sessionId, techniqueIds),
    onSuccess: (_data, { sessionId }) => {
      queryClient.invalidateQueries({ queryKey: ['sessions', sessionId, 'techniques'] });
    },
    ...
    onError: error =>
      showToast(
        getApiErrorMessage(error, 'Session saved, but techniques could not be linked'),
        'error',
      ),
  });
}
